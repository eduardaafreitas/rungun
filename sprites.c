...
void sprites_player (ALLEGRO_BITMAP *sheet, player *soldier){
    for (int i = 0; i < 6; i++) {
        soldier->sprites_player[i] = al_create_sub_bitmap(sheet, i * 40, 0, 40, 40);
        if (soldier->sprites_player[i] == NULL) {
            printf("Erro ao criar sub_bitmap para o sprite %d\n", i);
            // Liberar bitmaps alocados anteriormente em caso de erro
            for (int j = 0; j < i; j++) {
                al_destroy_bitmap(soldier->sprites_player[j]);
            }
            exit(1);
        }
